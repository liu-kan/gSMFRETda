...
void mc::init_randstate(int N,int sid){
    ...
    // cudaFree ( devStates[sid]) returned an illegal memory access was encountered(700) at /home/liuk/data/proj/gSMFRETda/src/mc.cu:303
        ...
        devQStates[sid]= (curandStateScrambledSobol64*)(mr->malloc(NN*sizeof (curandStateScrambledSobol64 ),streams[sid]));
        // CUDA_CHECK_RETURN(cudaMalloc ( (void **)&(devStates[sid]), NN*sizeof (rk_state ) ));
        // CUDA_CHECK_RETURN(cudaMalloc ( (void **)&(devQStates[sid]), NN*sizeof( curandStateScrambledSobol64)  ));    
        
        // CUDA_CHECK_RETURN(cudaFree (devDirectionVectors64[sid]));
        // CUDA_CHECK_RETURN(cudaFree (devScrambleConstants64[sid]));
        mr->free(devDirectionVectors64[sid],oldNN* sizeof(curandDirectionVectors64_t),streams[sid]);
        mr->free(devScrambleConstants64[sid],oldNN*sizeof(unsigned long long int),streams[sid]);
        devDirectionVectors64[sid]= (unsigned long long int*)(mr->malloc(NN * VECTOR_SIZE * sizeof(long long int),streams[sid]));
        devScrambleConstants64[sid]= (unsigned long long int*)(mr->malloc(NN*sizeof(long long int),streams[sid]));
        ...
        */
        ...
        while (NN > 0) {
            ...
            CUDA_CHECK_RETURN(cudaMemcpyAsync(buf+n*20000*sizeof(curandDirectionVectors64_t)/sizeof(unsigned long long int),
                hostVectors64,
...
void mc::init_data_gpu(vector<int64_t>& istart,vector<int64_t>& start,vector<int64_t>& stop,
    ...
    sz_burst=start.size();         
    int sidx=0;
    g_phCount=(int *)mr->malloc(sizeof(int)*sz_burst,streams[(sidx)%streamNum]);
    CUDA_CHECK_RETURN(cudaMemcpyAsync(g_phCount, phCount.data(), sizeof(int)*sz_burst,cudaMemcpyHostToDevice,streams[(sidx++)%streamNum]));
    g_burst_ad=(int64_t *)mr->malloc(sizeof(int64_t)*sz_tag,streams[(sidx)%streamNum]);
    CUDA_CHECK_RETURN(cudaMemcpyAsync(g_burst_ad, burst_ad, sizeof(int64_t)*sz_tag,cudaMemcpyHostToDevice,streams[(sidx++)%streamNum]));
    g_burst_dd=(int64_t *)mr->malloc(sizeof(int64_t)*sz_tag,streams[(sidx)%streamNum]);
    CUDA_CHECK_RETURN(cudaMemcpyAsync(g_burst_dd, burst_dd, sizeof(int64_t)*sz_tag,cudaMemcpyHostToDevice,streams[(sidx++)%streamNum]));

    g_istart=(int64_t *)mr->malloc(sizeof(int64_t)*sz_burst,streams[(sidx)%streamNum]);
    CUDA_CHECK_RETURN(cudaMemcpyAsync(g_istart, istart.data(), sizeof(int64_t)*sz_burst,cudaMemcpyHostToDevice,streams[(sidx++)%streamNum]));
    // CUDA_CHECK_RETURN(cudaMalloc((void **)&g_start, sizeof(int64_t)*sz_burst));
    g_start=(int64_t *)mr->malloc(sizeof(int64_t)*sz_burst,streams[(sidx)%streamNum]);
    CUDA_CHECK_RETURN(cudaMemcpyAsync(g_start, start.data(), sizeof(int64_t)*sz_burst,cudaMemcpyHostToDevice,streams[(sidx++)%streamNum]));
    // CUDA_CHECK_RETURN(cudaMalloc((void **)&g_stop, sizeof(int64_t)*sz_burst));
    g_stop=(int64_t *)mr->malloc(sizeof(int64_t)*sz_burst,streams[(sidx)%streamNum]);
    CUDA_CHECK_RETURN(cudaMemcpyAsync(g_stop, stop.data(), sizeof(int64_t)*sz_burst,cudaMemcpyHostToDevice,streams[(sidx++)%streamNum]));
          
    // CUDA_CHECK_RETURN(cudaMalloc((void **)&g_burst_duration, sizeof(float)*sz_burst));
    g_burst_duration=(float *)mr->malloc(sizeof(float)*sz_burst,streams[(sidx)%streamNum]);
    CUDA_CHECK_RETURN(cudaMemcpyAsync(g_burst_duration, T_burst_duration.data(), sizeof(float)*sz_burst,cudaMemcpyHostToDevice,streams[(sidx++)%streamNum]));      
    // CUDA_CHECK_RETURN(cudaMalloc((void **)&g_SgDivSr, sizeof(float)*sz_burst));
    g_SgDivSr=(float *)mr->malloc(sizeof(float)*sz_burst,streams[(sidx)%streamNum]);
    CUDA_CHECK_RETURN(cudaMemcpyAsync(g_SgDivSr, SgDivSr.data(), sizeof(float)*sz_burst,cudaMemcpyHostToDevice,streams[(sidx++)%streamNum]));      
...
int mc::setBurstBd(int cstart,int cstop, int sid){
    ...
    if(end_burst[sid]-begin_burst[sid]!=N){
        ...
        mcE[sid]=(retype*)(mr->malloc( N *reSampleTimes* sizeof(retype),streams[sid]));
        if (debug)  
            cout<<"mcE[sid]"<<sid<<":"<<mcE[sid]<<endl;   
...
void mc::free_data_gpu(){            
    ...
    mr->free(g_burst_duration,sizeof(float)*sz_burst,streams[(sidx++)%streamNum]);    
...
bool mc::set_nstates(int n,int sid){
    ...
    if (s_n[sid]!=n){
        ...
        // if (gpe[sid]!=0) CUDA_CHECK_RETURN(cudaFree(gpe[sid])); 
        // if (gpv[sid]!=0) CUDA_CHECK_RETURN(cudaFree(gpv[sid]));
        // if (gpp[sid]!=0) CUDA_CHECK_RETURN(cudaFree(gpp[sid]));
        // if (gpk[sid]!=0) CUDA_CHECK_RETURN(cudaFree(gpk[sid]));
        mr->free(gpe[sid],s_n[sid]*sizeof(float),streams[sid]);
        mr->free(gpv[sid],s_n[sid]*sizeof(float),streams[sid]);
        mr->free(gpp[sid],s_n[sid]*sizeof(float),streams[sid]);
        mr->free(gpk[sid],s_n[sid]*sizeof(float),streams[sid]);
        std::cout<<"&(gpe["<<sid<<"]):"<<(void**)&(gpe[sid])<<"\tn:"<<n<<std::endl;
