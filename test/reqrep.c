  ...
  while (1)
    {
      ...
      else if (strncmp (ID1, buf, sz_date) == 0)
        {
          printf ("NODE0: RECEIVED id1 REQUEST\n");
          char *d = "date() id1";
          int sz_d = strlen(d) + 1; // '\0' too
          printf ("NODE0: SENDING DATE %s\n", d);
          bytes = nn_send (sock, d, sz_d, 0);
      ...
      else if (strncmp (ID2, buf, sz_date) == 0)
        {
          printf ("NODE0: RECEIVED id2 REQUEST\n");
          char *d = "date() id2";
          int sz_d = strlen(d) + 1; // '\0' too
          printf ("NODE0: SENDING DATE %s\n", d);
          bytes = nn_send (sock, d, sz_d, 0);
...
int node1 (const char *url,const char *id)
{
  int sz_date = strlen(id) + 1; // '\0' too
  char *buf = NULL;
  int bytes = -1;
  ...
  while(1){
    ...
    bytes = nn_send (sock, id, sz_date, 0);
